...
__all__ = ["float_quantize_nearest_cuda", "float_quantize_stochastic_cuda"]
...
def float_quantize_stochastic_cuda(a, man_bits, exp_bits, subnormals, saturate):
    res = _float_quantize_stochastic_cuda_prim.bind(
        a,
        np.array([man_bits, exp_bits], dtype=np.int64),
        np.array([subnormals, saturate], dtype=bool),
    )
...
def _float_quantize_stochastic_cuda_abstract(a, format, features):
    shape = a.shape
    dtype = dtypes.canonicalize_dtype(a.dtype)
    return ShapedArray(shape, dtype)


def _float_quantize_stochastic_cuda_translation(c, a, format, features, *, platform="gpu"):
    assert platform == "gpu"
    # The inputs have "shapes" that provide both the shape and the dtype
    a_shape = c.get_shape(a)
    format_shape = c.get_shape(format)
    ...
    op_name = "float_quantize_stochastic_cuda".encode()
...
_float_quantize_stochastic_cuda_prim = core.Primitive("float_quantize_nearest_cuda")
_float_quantize_stochastic_cuda_prim.def_impl(
    partial(xla.apply_primitive, _float_quantize_stochastic_cuda_prim)
)
_float_quantize_stochastic_cuda_prim.def_abstract_eval(
    _float_quantize_stochastic_cuda_abstract
)

xla.backend_specific_translations["gpu"][_float_quantize_stochastic_cuda_prim] = partial(
    _float_quantize_stochastic_cuda_translation, platform="gpu"
)
