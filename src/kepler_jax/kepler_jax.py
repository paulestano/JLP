...
__all__ = ["kepler", "float_quantize_nearest_cuda"]
...
def float_quantize_nearest_cuda( a, man_bits, exp_bits, subnormals, saturate):
    print("calling quantization func")
    res = _float_quantize_nearest_cuda_prim.bind( a, man_bits, exp_bits, subnormals, saturate)
    print(res)
    return res

def _float_quantize_nearest_cuda_abstract(a, man_bits, exp_bits, subnormals, saturate):
    shape = a.shape
    dtype = dtypes.canonicalize_dtype(a.dtype)
    return (ShapedArray(shape, dtype))

def _float_quantize_nearest_cuda_translation(c, a, man_bits, exp_bits, subnormals, saturate , *, platform="gpu"):
    assert platform == "gpu"
    # The inputs have "shapes" that provide both the shape and the dtype
    ...
    scalar_shape =xla_client.Shape.array_shape(np.dtype(np.int64), (), ())
    bool_shape =xla_client.Shape.array_shape(np.dtype(bool), (), ())
    ...
    return xops.CustomCallWithLayout(
        c,
        op_name,
        operands=(a, man_bits, exp_bits, subnormals, saturate),
        operand_shapes_with_layout=(shape, scalar_shape, scalar_shape, bool_shape, bool_shape),
        shape_with_layout=shape,
        opaque=opaque,
    )

_float_quantize_nearest_cuda_prim = core.Primitive("float_quantize_nearest_cuda")
_float_quantize_nearest_cuda_prim.def_impl(partial(xla.apply_primitive, _float_quantize_nearest_cuda_prim))
_float_quantize_nearest_cuda_prim.def_abstract_eval(_float_quantize_nearest_cuda_abstract)
